...
export default async function fetchListingsFromSheets () {
  try {
    ...
    return rows.map((row, index) => ({
      ...
      title: row[0],
      description: row[1] || "",
      LongTags: row[2] ? row[2].split(",").map(tag => tag.trim()) : [],   // L: LongTags (12-та колонка, індекс 11)
      ShortTags: row[3] ? row[3].split(",").map(tag => tag.trim()) : [], // M: ShortTags (13-та колонка, індекс 12)
    }));
  } catch (err) {
